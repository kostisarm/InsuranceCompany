	...
	public static float Client_bill(String Client_name) {
		...
		try {
			...
			while (rs1.next()) {
				...
				while (rs2.next()) {
					...
					if (ins_dur == 6) {
						String sql = "UPDATE Client_insurances SET insurance_cost =" + ins_cost + "where lastName ='"
								+ clientname + "' and insurance_type = '" + ins_type + "' and insurance_duration="
								+ ins_dur;
						stmt3.executeUpdate(sql);
					} else {
						String sql = "UPDATE Client_insurances SET insurance_cost =" + 1.8 * ins_cost
								+ "where lastName ='" + clientname + "' and insurance_type = '" + ins_type
								+ "' and insurance_duration=" + ins_dur;
						stmt3.executeUpdate(sql);
					}
				}
				rs2.close();
		...
		try {
			...
			rs2 = stmt2.executeQuery(
					"SELECT sum(insurance_cost) as sum FROM Client_insurances WHERE lastName ='" + clientname + ("'"));
			rs2.next();
